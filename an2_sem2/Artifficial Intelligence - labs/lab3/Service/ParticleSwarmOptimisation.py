...
class ParticleSwarmOptiAlgorithm(object):
    def __init__(self, n, noParticles, sizeOfNeighbourhood , w, c1, c2, nrOfTrials):
        self.inertialCoeffW=w #how big is the place that you explore
        self.cognitiveFactorC1=c1 #how much to go untill best position
        self.socialFactorC2=c2 #biggest, that much it will move to a greatest neighbour
        self.n=n
    ...
    def selectNeighbors(self):
        ...
        if(self.sizeOfNeighbourhood==len(self.population)):
            neighbors = []
            for i in range(len(self.population)): #every particle in the swarm (every matrix)
                localNeighbor = []
                 #create neighbours for the particle
                x=np.random.permutation(len(self.population)) #add whole population as positions in the neighbour
        ...
        # find best neighbour
        # keep in mind positions from population (not matrixes; memory purpose)
        neighbors = []
    ...
    def iteration(self,w):
        ...
        pop=self.population
        bestNeighbors = []
        # determine the best neighbour for each particle
