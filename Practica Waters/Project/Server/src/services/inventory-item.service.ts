...
async function getInventoryItems(
  ...
  else
  {
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, 500));
      return items.filter( item => (item.user==filter  || item.location==filter || 
                                    item.name==filter || item.description == filter || 
                                    item.inventoryNumber.toString() == filter || item.id==filter));
  }catch (ex) {
    return ex;
  }
  }
